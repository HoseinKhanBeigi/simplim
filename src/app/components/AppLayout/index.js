...
const AppLayout = () => {
  ...
  const [insights, setInsights] = useState([{ content: 'Select text to get insights.', source: null }]);
  ...
  const handleClarification = async (text) => {
    if (user.role === 'premium' || user.clarificationsUsed < user.clarificationsLimit) {
      try {
      ...
      toast.error('Clarification limit reached. Upgrade to Premium for unlimited access!');
      return false;
    }
  };
  ...
  const handleSavePDF = async (content) => {
    if (!user.role === 'premium' && pdfsEdited >= MAX_FREE_PDFS) {
      toast.error('Free tier limit reached. Upgrade to continue creating PDFs!');
      return;
    }
  ...
  const handleUpgrade = async () => {
    try {
      ...
      toast.success('Welcome to Premium! Enjoy unlimited PDF creation and advanced features.');
    } catch (error) {
